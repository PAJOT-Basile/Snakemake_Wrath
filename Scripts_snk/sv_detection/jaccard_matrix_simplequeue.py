...
parser.add_argument("-w", "--winFile",
                    help="Input window file", action="store")
parser.add_argument("-b", "--barcodeFile",
                    help="Input barcode file", action="store")
parser.add_argument("-o", "--outFile",
                    help="Output jaccard matrix file", action="store")

# other
parser.add_argument("-t", "--threads", help="Analysis threads",
                    type=int, action="store", default=1)
parser.add_argument("--test", help="Test - runs 10 windows",
                    action='store_true')
...
# read windows
windowFile = pd.read_csv(args.winFile, sep='\t',
                         lineterminator='\n', header=None)
...
def freqs_wrapper(inQueue, resultQueue, number_win, inFile):
    while True:
        ...
        if windowNumber == -1:
            # this is the way of telling everything we're done
            resultQueue.put((-1, None,))
            break
        array_i = np.zeros((number_win))
        array_u = np.ones((number_win))
        bedfile1 = inFile.fetch(
            windowLine[0], windowLine[1], windowLine[2],  parser=pysam.asBed(), multiple_iterators=True)
        barcodes1 = [rowbed1.name for rowbed1 in bedfile1]
        for index2, row2 in windowFile.iloc[windowNumber:, :].iterrows():
            bedfile2 = inFile.fetch(
                row2[0], row2[1], row2[2], parser=pysam.asBed(), multiple_iterators=True)
...
def sorter(doneQueue, writeQueue, verbose, nWorkerThreads):
    ...
    sortBuffer = {}
    expect = 0
    # this will keep track of the worker threads and once they're all done this thread will break
    threadsComplete = 0
    while True:
...
for x in range(args.threads):
    workerThread = Process(target=freqs_wrapper, args=(
        inQueue, resultQueue, num_win, tbx,))
...
sorterThread = Thread(target=sorter, args=(
    resultQueue, writeQueue, args.verbose, args.threads,))
