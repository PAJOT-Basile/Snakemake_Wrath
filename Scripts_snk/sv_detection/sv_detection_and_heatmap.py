...
parser.add_argument("-o", "--outliers",
                    help="Input detected outliers", action="store")
...
parser.add_argument("-f", "--winSize", help="Window size",
                    type=int, action="store", default=1)
parser.add_argument("-c", "--chromosome",
                    help="Chromosome name", type=str, action="store")
parser.add_argument("-w", "--windowFile",
                    help="Input genomic windows file", action="store")
...
matrix_file = pd.read_csv(args.matrix, sep=',',
                          lineterminator='\n', header=None)
outliers_file = pd.read_csv(args.outliers, sep=',', lineterminator='\n')
window_file = pd.read_csv(args.windowFile, sep='\t',
                          lineterminator='\n', header=None)
outplot = args.plot
output = args.outFile
...
clustering = AgglomerativeClustering(
    n_clusters=None, distance_threshold=3, linkage='single').fit(points)
outliers_file.loc[:, 'group'] = clustering.labels_

# Identification of breakpoints
groups = outliers_file.groupby(['group'])
d = {'mincol': groups.min(['ncol'])['ncol'], 'maxcol': groups.max(['ncol'])[
    'ncol'], 'minrow': groups.min(['nrow'])['nrow'], 'maxrow': groups.max(['nrow'])['nrow']}
breakPoints = pd.DataFrame(data=d)

# plot heatmap in half a triangle and the detected outliers in the other
sns.set(font_scale=3)
print("Plotting SVs")
# transform the data to log scale and multiply by 100 to get a percentage
data = np.log(matrix_file + 0.0001) * 100

heatmap_plot = sns.heatmap(data, cmap="YlGnBu", square=True, cbar_kws={
                           'label': 'Barcode sharing %', 'shrink': 0.5})
heatmap_plot.scatter(x=breakPoints['minrow'],
                     y=breakPoints['mincol'], color='k')
heatmap_plot.scatter(x=breakPoints['maxrow']+1,
                     y=breakPoints['maxcol']+1, color='m')
...
if num_ticks_y <= 10:  # if there are less than 10 ticks, set the number of ticks to 10
    num_ticks_y = 10
tick_locs_y = np.around(np.linspace(
    0, len(data.index) - 1, num_ticks_y)).astype(int)
# get the labels from the index (chromosome positions)
...
# Adjust the number of desired ticks
num_ticks_x = round(len(data.columns) / 60)
if num_ticks_x <= 10:  # if there are less than 10 ticks, set the number of ticks to 10
    num_ticks_x = 10
tick_locs_x = np.around(np.linspace(
    0, len(data.columns) - 1, num_ticks_x)).astype(int)
# get the labels from the index (chromosome positions)
...
tick_locs = np.linspace(np.nanmin(data.to_numpy()),
                        np.nanmax(data.to_numpy()), num_ticks)
...
print("Saving SVs detection")
breakPoints['length'] = breakPoints['maxcol']-breakPoints['minrow']
breakPoints.sort_values(by=['length'], ascending=False, inplace=True)

output_table = {'SV_id': breakPoints.index.values, 'chromosome': chrom,
                'start': breakPoints['minrow']*window_size, 'end': breakPoints['maxcol']*window_size, 'length': breakPoints['length']*window_size}
